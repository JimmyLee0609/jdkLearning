...
public class ByteBufferTest {
	...
	public static void main(String[] args) {
		...
		ByteBuffer wrap2 = ByteBuffer.wrap("test".getBytes(),2,1);//依然保存这个数组全部内容  position 2 limit 3
		int arrayOffset = wrap2.arrayOffset();//0
		silce();
		byteBuffer();
		// 大端，小端的标志
		byteOrder();
	...
	private static void shareCache() {
		...
		// 创建共享此缓冲区内容的新的字节缓冲区。      缓存内容共享，保留各自的标记为的操作
	...
	private static void cacheFlag(ByteBuffer wrap) {
		...
		// 如果已定义了标记，则丢弃该标记。
		Buffer flip = wrap.flip();// 翻动 用在写出前 ，limit=position; position=0;
									// mark=-1; 准备好为流，管道
	...
	private static void byteBuffer() {
		...
		// 直到小的remain为止（Math.min(this.remaining(),
		// that.remaining());），如果不一样，返回compare的值
		int compareTo = wrap.compareTo(wrap);

		byte[] dst = new byte[] { 12, 53, 64, 65, 9, 51, 15, 15, 8, 5, 4, 32, 13, 2, 21, 6, 5, 4, 55, 4, 63, 2, 15, 6,
				2, 1, 6, 5, 1, 3, 5, 1 };
	...
	private static void toOtherBuffer(ByteBuffer wrap) {
		// 将缓冲转换为其他类型
		CharBuffer asCharBuffer = wrap.asCharBuffer();//也可以通过charset的enCode来转换，这个比较特殊
	...
	private static void singlePutGet(ByteBuffer wrap) {
		...
		// 写入操作直接从position位置开始，向后覆盖数据，改变position的值
