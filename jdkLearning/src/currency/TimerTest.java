...
//		计时器类     并发包有的SchedulePool替代了  不过这个方法简单易懂
		Timer timer = new Timer();                         //新建一个计时器   名称系统默认   this("Timer-" + serialNumber());
//		Timer timer2 = new Timer("first");				//新建一个自定义名称的计时器
//		Timer timer3 = new Timer(false);				//根据传入的布尔值  新建计时器是否是后台线程  名字系统默认   this("Timer-" + serialNumber(), isDaemon);
